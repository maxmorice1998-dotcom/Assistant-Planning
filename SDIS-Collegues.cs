...
internal sealed class MainForm:Form {
 ...
 int displayedSyncProgress;
 int targetSyncProgress;
 ...
 async void Call(string action){
  if(busy)return;SetBusy(true,action=="startup"||action=="update-check"?"Recherche de mise \u00e0 jour...":BusyMessage(action),action=="synchronize",action=="startup"||action=="update-check"||action=="update-install");
  ...
  try{
   ...
   if(Flag(response,"updated")){status.Text="Mise a jour d'Assistant Planning...";Application.Exit();return;}
    ...
    if(Flag(response,"warning"))ShowError("Mise \u00e0 jour impossible. La version actuelle reste utilisable. Consultez le journal pour conna\u00eetre la cause.");
    else status.Text="";
...
void RefreshStatusInBackground(){
 ...
 void PollUpdateProgress(){try{string file=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"SDIS-Bot-Collegues","update-progress.json");if(!File.Exists(file))return;var p=json.Deserialize<Dictionary<string,object>>(File.ReadAllText(file,Encoding.UTF8));DateTime updated;if(p==null||!p.ContainsKey("updatedAt")||!DateTime.TryParse(Convert.ToString(p["updatedAt"]),out updated)||updated.ToUniversalTime()<syncStartedAt.ToUniversalTime().AddSeconds(-1))return;busyText.Text="Mise \u00e0 jour en cours"+(p.ContainsKey("version")?" \u2014 v"+Convert.ToString(p["version"]):"");if(p.ContainsKey("stage"))progressDetail.Text=Convert.ToString(p["stage"]);double downloaded=p.ContainsKey("downloaded")&&p["downloaded"]!=null?Convert.ToDouble(p["downloaded"]):0,total=p.ContainsKey("total")&&p["total"]!=null?Convert.ToDouble(p["total"]):0;if(total>0){syncProgress.Style=ProgressBarStyle.Continuous;syncProgress.Value=(int)Math.Max(0,Math.Min(100,downloaded*100/total));progressCounters.Text=(downloaded/1048576).ToString("0.0")+" Mo / "+(total/1048576).ToString("0.0")+" Mo \u2014 "+syncProgress.Value+" %";}else{syncProgress.Style=ProgressBarStyle.Marquee;progressCounters.Text=downloaded>0?(downloaded/1048576).ToString("0.0")+" Mo t\u00e9l\u00e9charg\u00e9s":"";}progressDuration.Text="Merci de patienter. Ne fermez pas Assistant Planning.";}catch{}}
 void PollProgress(){if(updateProgressMode){PollUpdateProgress();return;}if(!syncProgressMode)return;try{string file=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"SDIS-Bot-Collegues","sync-progress.json");if(File.Exists(file)){var p=json.Deserialize<Dictionary<string,object>>(File.ReadAllText(file,Encoding.UTF8));DateTime written;if(p!=null&&String.Equals(Convert.ToString(p.ContainsKey("mode")?p["mode"]:""),"sync",StringComparison.OrdinalIgnoreCase)&&p.ContainsKey("at")&&DateTime.TryParse(Convert.ToString(p["at"]),out written)&&written.ToUniversalTime()>=syncStartedAt.ToUniversalTime().AddSeconds(-1)){if(p.ContainsKey("percent")){int n=Math.Max(0,Math.Min(100,Convert.ToInt32(p["percent"])));targetSyncProgress=Math.Max(targetSyncProgress,n);}if(p.ContainsKey("message")&&!String.IsNullOrWhiteSpace(Convert.ToString(p["message"])))progressDetail.Text=Convert.ToString(p["message"]);if(p.ContainsKey("detail"))progressCounters.Text=Convert.ToString(p["detail"]);}}if(displayedSyncProgress<targetSyncProgress)displayedSyncProgress++;syncProgress.Value=displayedSyncProgress;busyText.Text="Synchronisation en cours — "+displayedSyncProgress+" %";if(syncStartedAt!=DateTime.MinValue)progressDuration.Text="Durée : "+(DateTime.Now-syncStartedAt).TotalSeconds.ToString("0.0")+" secondes";}catch{}}
 void SetBusy(bool value,string message,bool showProgress,bool update=false){busy=value;updateProgressMode=value&&update;syncProgressMode=value&&(showProgress||update);syncProgress.Style=update?ProgressBarStyle.Marquee:ProgressBarStyle.Continuous;busyText.Text=message;busyPanel.Visible=syncProgressMode;syncProgress.Visible=syncProgressMode;progressDetail.Visible=syncProgressMode;progressCounters.Visible=syncProgressMode;progressDuration.Visible=syncProgressMode;if(value){syncStartedAt=DateTime.Now;displayedSyncProgress=0;targetSyncProgress=0;syncProgress.Value=0;progressDetail.Text="";progressCounters.Text="";progressDuration.Text="";busyTimer.Tag=0;busySpinner.Text="|";busyTimer.Start();if(syncProgressMode){progressTimer.Start();busyPanel.BringToFront();}}else{busyTimer.Stop();progressTimer.Stop();busyPanel.Visible=false;}foreach(Button b in buttons){b.Visible=!value&&!calendarPreview.Visible;b.Enabled=!value;}reposCard.Visible=!value&&!calendarPreview.Visible;reposCheck.Visible=!value&&!calendarPreview.Visible;reposCheck.Enabled=!value;}
