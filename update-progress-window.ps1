param([Parameter(Mandatory=$true)][string]$DoneFile,[Parameter(Mandatory=$true)][string]$ProgressFile,[Parameter(Mandatory=$true)][string]$Version,[string]$LogoFile='')
...
$mark=[Windows.Forms.PictureBox]::new();$mark.SetBounds(26,18,92,88);$mark.SizeMode='Zoom';$mark.BackColor=[Drawing.Color]::Transparent;if($LogoFile-and(Test-Path -LiteralPath $LogoFile)){try{$bytes=[IO.File]::ReadAllBytes($LogoFile);$stream=[IO.MemoryStream]::new($bytes);$source=[Drawing.Image]::FromStream($stream);$mark.Image=[Drawing.Bitmap]::new($source);$source.Dispose();$stream.Dispose()}catch{}};$header.Controls.Add($mark)
...
$progress=[Windows.Forms.ProgressBar]::new();$progress.SetBounds(70,245,430,18);$progress.Minimum=0;$progress.Maximum=100;$progress.Value=0;$progress.Style='Continuous';$panel.Controls.Add($progress)
...
$frames=@('|','/','-','\');$index=0
$timer=[Windows.Forms.Timer]::new();$timer.Interval=120;$timer.Add_Tick({if(Test-Path -LiteralPath $DoneFile){$timer.Stop();try{[IO.File]::Delete($DoneFile)}catch{};$form.Close();return};if(Test-Path -LiteralPath $ProgressFile){try{$state=Get-Content -LiteralPath $ProgressFile -Raw|ConvertFrom-Json;if($null-ne$state.percent){$progress.Value=[Math]::Max(0,[Math]::Min(100,[int]$state.percent));$title.Text='Installation en cours - v'+$Version+' - '+$progress.Value+' %'};if($state.stage){$detail.Text=[string]$state.stage}}catch{}};$script:index=($script:index+1)%$frames.Count;$spinner.Text=$frames[$script:index]});$timer.Start()
