...
function acquireLock(file){fs.mkdirSync(path.dirname(file),{recursive:true});if(fs.existsSync(file)){let active=false;try{const meta=JSON.parse(fs.readFileSync(file,"utf8"));active=pidAlive(meta.pid);}catch{}if(active)throw new Error("Mise à jour déjà en cours.");try{fs.unlinkSync(file);}catch{throw new Error("Ancien verrou de mise à jour impossible à supprimer.");}}const fd=fs.openSync(file,"wx");fs.writeFileSync(fd,JSON.stringify({pid:process.pid,date:new Date().toISOString(),type:"update"}),"utf8");return fd;}
